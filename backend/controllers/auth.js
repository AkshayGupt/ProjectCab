...
exports.signup = (req, res) => {
  ...
  User.findOne({ email }, (err, user) => {
      ...
      const token = jwt.sign({email,password,firstName,lastName}, process.env.ACTIVATION_KEY, {expiresIn:'15m'});
        ...
        const {email,password,firstName,lastName} =decodedToken;
              ...
              User.findOne({ email }, (err, user) => {
                  ...
                  newUser.save((err, user) => {
                    ...
                    return res.status(200).json({ name: user.firstName, email: user.email, bio:user.bio });
                  });
